...
const CancelledTrades = ({ setData, setOpenTradeDrawer }) => {
  const queryClient = useQueryClient();
  const { userInformation } = useSelector((state)=>state.authReducer.userData)

  const fetchCancelledTrades = async ({ pageParam = 0 } = {}) =>{
    const response =  await axios.get(`https://skillswap-server.onrender.com/trade/cancelled/${userInformation._id}?cursor=${pageParam}`)
  ...
  const {
    ...
    queryKey: ['cancelledTrades'],
    queryFn: fetchCancelledTrades,
  ...
  useEffect(() => {
    ...
    return () => {
      queryClient.removeQueries(['cancelledTrades']);
    };
  }, [userInformation._id]);
  ...
  const groupedData = data.pages.reduce((acc, page) => {
    page.CancelledTrades.forEach((trade) => {
          ...
          <React.Fragment key={index}>
            ...
            {trades.map((trade, tradeIndex) => {
              return(
                ...
                onClick={()=>{ setOpenTradeDrawer((prev)=>!prev); setData(trade)}}
                ref={tradeIndex === trades.length - 1 ? ref : null}
                key={tradeIndex}
                >
                  <div className="w-2 h-full bg-rose-500 rounded-2xl"></div>
                  ...
                  <div className="w-1/3 flex justify-end">
                    <div className="flex flex-col items-end gap-2 ">
                      <div className={`mb-2 rounded-md px-2 py-1 flex items-center justify-center text-xs  font-main dark:bg-rose-800/20 bg-rose-400/10 `}>
                        <span className={`font-main text-xs font-medium dark:font-medium dark:text-rose-500 text-rose-500`}>
                          Cancelled
                        </span>
                      </div>
                    </div> 
