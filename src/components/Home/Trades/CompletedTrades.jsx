...
const CompletedTrades = ({setData, setOpenTradeDrawer, setToggleEdit }) => {
  const queryClient = useQueryClient();
  const { userInformation } = useSelector((state)=>state.authReducer.userData)

  const fetchCompletedTrades = async ({ pageParam = 0 } = {}) =>{
    const response =  await axios.get(`https://skillswap-server.onrender.com/trade/completed/${userInformation._id}?cursor=${pageParam}`)
  ...
  useEffect(() => {
    ...
    return () => {
      queryClient.removeQueries(['completedTrades']);
    };
  }, [userInformation._id]);
  ...
  const groupedData = data.pages.reduce((acc, page) => {
    page.CompletedTrades.forEach((trade) => {
          ...
          <React.Fragment key={index}>
            ...
            {trades.map((trade, tradeIndex) => {
              return(
                ...
                key={tradeIndex}
                >
                  <div className="w-2 h-full bg-neutral-500 rounded-2xl"></div>
                  ...
                  <div className="w-1/3 flex justify-end">
                    <div className="flex flex-col items-end gap-2 ">
                      <div className={`mb-2 rounded-md px-2 py-1 flex items-center justify-center text-xs  font-main dark:bg-neutral-800 bg-neutral-400/10`}>
                        <span className={`font-main text-xs font-medium dark:font-medium dark:text-neutral-500 text-neutral-500`}>
                          Completed
                        </span>
                      </div>
                    </div> 
