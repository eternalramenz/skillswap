...
const AcceptedTrades = ({ setData, setOpenTradeDrawer, setToggleEdit }) => {
  ...
  const fetchAcceptedTrades = async ({ pageParam = 0 } = {}) =>{
    const response =  await axios.get(`https://skillswap-server.onrender.com/trade/accepted/${userInformation._id}?cursor=${pageParam}`)
  ...
  const {
    ...
    queryKey: ['acceptedTrades'],
    queryFn: fetchAcceptedTrades,
  ...
  useEffect(() => {
    fetchAcceptedTrades();
    return () => {
      queryClient.removeQueries(['acceptedTrades']);
    };
  }, [userInformation._id]);
  ...
  const groupedData = data.pages.reduce((acc, page) => {
    page.OngoingTrades.forEach((trade) => {
          ...
          <React.Fragment key={index}>
            ...
            {trades.map((trade, tradeIndex) => {
              return(
                ...
                key={tradeIndex}
                >
                  <div className="w-2 h-full bg-green-500 rounded-2xl"></div>

                  <div className="flex flex-col justify-center items-center px-4 h-full w-1/3">
                    <span className='font-main text-xl font-medium dark:text-white2'>{convertWeekDay(trade.date)}</span>
                    <span className='font-main text-sm dark:text-white2 text-darkBlue  px-2'>{(trade.time).split(" — ")[0]}</span>
                  </div>
                  ...
                  <div className="w-1/3 flex justify-end">
                    <div className="flex flex-col items-end gap-2 ">
                      <div className={`mb-2 rounded-md px-2 py-1 flex items-center justify-center text-xs  font-main dark:bg-green-800/20 bg-green-400/10 `}>
                        <span className={`font-main text-xs font-medium dark:font-medium dark:text-green-500 text-green-500`}>
                          Accepted
                        </span>
                      </div>
                    </div> 
